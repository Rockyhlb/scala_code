...
*/
  ...
  //      列表中的初始元素是按照插入的先后顺序来组织的,后续新插入元素无序，
  //      但是，“集合”中的元素并不会记录元素的插入顺序，而是以“哈希”方法
  //      对元素的值进行组织,所以，它允许你快速地找到某个元素
  //        ●集合包括可变集和不可变集，分别位于
  ...
  //    println(firstMap(1))
  //    println(firstMap(2))
  //    // 以上方式如果给定的key不存在聚会抛出异常，因此可以先调用contains方法确定key是否存在
